export function mountUniverse({content:c,dialog},{el,projects,button,tag,track,record,contact,onProject}) {
  ...
  const controls=el('div',undefined,'exp-actions');let listOnly=matchMedia('(max-width: 600px)').matches;const viewport=el('div',undefined,'exp-universe-viewport');viewport.setAttribute('aria-label','Interactive project map. Use the project list below for keyboard navigation.');
  ...
  viewport.hidden=listOnly;viewport.append(world);const status=el('p');status.setAttribute('role','status');const list=el('div',undefined,'exp-project-list');
  ...
  const toggle=button(listOnly?'Show interactive map':'Use list view',()=>{listOnly=!listOnly;viewport.hidden=listOnly;toggle.textContent=listOnly?'Show interactive map':'Use list view';if(!listOnly)requestAnimationFrame(fit)});controls.append(toggle,button('− Zoom out',()=>zoom(.8)),button('+ Zoom in',()=>zoom(1.25)),button('Reset',()=>{selected='';select.value='';apply();fit()}),button('Exit',()=>dialog.close()));
